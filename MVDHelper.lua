...
imgui.OnInitialize(function()
    ...
    if fsClock == nil then
        ...
        local fontpath = path_join(getWorkingDirectory(), "lib", "mimgui", 'trebucbd.ttf')

        fsClock = imgui.GetIO().Fonts:AddFontFromFileTTF(fontpath, 25.0*MDS, nil, glyph_ranges)
    end
...
function main()
    ...
    if isPatrolActive then
      ...
      if not doesDirectoryExist(getWorkingDirectory()..'/arzfun') then createDirectory(getWorkingDirectory()..'/arzfun') end
      if not doesDirectoryExist(getWorkingDirectory()..'/MVDHelper') then createDirectory(getWorkingDirectory()..'/MVDHelper') end
      for i = 1, 2 do
            if not doesFileExist(getWorkingDirectory()..'/arzfun/'..i..'.png') then
                downloadFile("https://raw.githubusercontent.com/DanielBagdasarian/MVD-Helper-Mobile/refs/heads/main/arzfun/"..i..".png", getWorkingDirectory()..'/arzfun/'..i..".png")
            end
        end
...
local MainWindowPos = imgui.ImVec2(sizeX / 2, sizeY / 2 - 320)
local MainWindowSize = imgui.ImVec2(840 * MDS, 150 * MDS)
...
imgui.OnFrame(function() return menuSizes[0] end, function(player)
    ...
    if copMenu[0] then
        imgui.SliderInt(u8"Положение снежинки в таб баре", snegPos, 10, 500)
    else
        imgui.SliderInt(u8"Положение копа в таб баре", copPos, 10, 500)
    end
...
imgui.OnFrame(function() return vzWindow[0] end, function(player)
    ...
    imgui.Begin(u8'', vzWindow, imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoTitleBar)
...
local gunCommands = {
    ["3"] = "/me достал дубинку с поясного держателя",
    ["16"] = "/me взял с пояса гранату",
    ["17"] = "/me взял гранату слезоточивого газа с пояса",
    ["23"] = "/me достал тайзер с кобуры, убрал предохранитель",
    ["22"] = "/me достал пистолет Colt-45, снял предохранитель",
    ["24"] = "/me достал Desert Eagle с кобуры, убрал предохранитель",
    ["25"] = "/me достал чехол со спины, взял дробовик и убрал предохранитель",
    ["26"] = "/me резким движением обоих рук, снял военный рюкзак с плеч и достал Обрезы",
    ["27"] = "/me достал дробовик Spas, снял предохранитель",
    ["28"] = "/me резким движением обоих рук, снял военный рюкзак с плеч и достал УЗИ",
    ["29"] = "/me достал чехол со спины, взял МП5 и убрал предохранитель",
    ["30"] = "/me достал карабин AK-47 со спины",
    ["31"] = "/me достал карабин М4 со спины",
    ["32"] = "/me резким движением обоих рук, снял военный рюкзак с плеч и достал TEC-9",
    ["33"] = "/me достал винтовку без прицела из военной сумки",
    ["34"] = "/me достал Снайперскую винтовку с военной сумки",
    ["43"] = "/me достал фотокамеру из рюкзака",
    ["0"] = "/me поставил предохранитель, убрал оружие"
}
...
function loadCommands()
    ...
    if file then
        local content = file:read("*a")
        file:close()
        gunCommands = decodeJson(content) or {}
    else
...
gunsWindow = imgui.new.bool(false)
imgui.OnFrame(function() return gunsWindow[0] end, function(player)
    ...
    for id, name in pairs(weapons) do        
        if imgui.Button(u8(name .. "(" .. id .. ")")) then
            selectedGun = id
            otInput = imgui.new.char[255](u8(gunCommands[id]))
        end
        if selectedGun ~= nil and selectedGun ~= "" and selectedGun == id then
            imgui.SameLine()
            ...
            if imgui.Button(u8"Сохранить", imgui.ImVec2(100, 50)) then
                gunCommands[selectedGun] = u8:decode(ffi.string(otInput))
                saveCommands()
...
adWindow = new.bool(false)
imgui.OnFrame(function() return adWindow[0] end, function(player)
    
    imgui.SetNextWindowPos(MainWindowPos, imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
    imgui.SetNextWindowSize(MainWindowSize, imgui.Cond.FirstUseEver)
    imgui.Begin('##ad', adWindow, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoBackground + imgui.WindowFlags.NoResize)
    imgui.SetWindowPosVec2(imgui.ImVec2(MainWindowPos.x, MainWindowPos.y - 200*MDS))  
    local tabSize = 40 - 10
    local Size = imgui.GetWindowSize()
    local tabSize = 50
    imgui.SetCursorPos(imgui.ImVec2(Size.x - 80, 5))
    if imgui.Button('X##..##adWindow::closebutton', imgui.ImVec2(50, 50)) then adWindow[0] = false end
    imgui.SetWindowSizeVec2(imgui.ImVec2(MainWindowSize.x - 15, 220 * MDS))
    imgui.DrawFrames(MyGif, imgui.ImVec2(Size.x - 10, Size.y - 100), FrameTime[0])    
    imgui.End()
end)
...
imgui.OnFrame(function() return window[0] end, function(player)
    ...
    if copMenu[0] then
        imgui.SetCursorPosX(mainIni.menuSettings.snegPos)
        imgui.SetCursorPosY(80)
        p = imgui.GetCursorScreenPos()
        animationsnowflake(p.x, p.y, 0xffffffff, 13, 10)
    else
        imgui.SetCursorPosX(mainIni.menuSettings.copPos)
        imgui.ImageURL("https://raw.githubusercontent.com/DanielBagdasarian/MVD-Helper-Mobile/refs/heads/main/police.png", imgui.ImVec2(119, 141), true)
    end
    ...
    if imgui.Button('X##..##Window::closebutton', imgui.ImVec2(50, 50)) then if window then window[0] = false adWindow[0] = false end end
    ...
    if page == 1 then -- если значение tab == 1
            ...
            lua_thread.create(function()
                while true do
                    ...
                    if lastgun ~= getCurrentCharWeapon(PLAYER_PED) then
                        ...
                        elseif gun == 22 then
                            sampSendChat(gunCommands["22"])
                        elseif gun == 24 then
                            sampSendChat(gunCommands["24"])
                        elseif gun == 25 then
                            sampSendChat(gunCommands["25"])
                        elseif gun == 26 then
                            sampSendChat(gunCommands["26"])
                        elseif gun == 27 then
                            sampSendChat(gunCommands["27"])
                        elseif gun == 28 then
                            sampSendChat(gunCommands["28"])
                        elseif gun == 29 then
                            sampSendChat(gunCommands["29"])
                        elseif gun == 30 then
                            sampSendChat(gunCommands["30"])
                        elseif gun == 31 then
                            sampSendChat(gunCommands["31"])
                        elseif gun == 32 then
                            sampSendChat(gunCommands["32"])
                        elseif gun == 33 then
                            sampSendChat(gunCommands["33"])
                        elseif gun == 34 then
                            sampSendChat(gunCommands["34"])
                        elseif gun == 43 then
                            sampSendChat(gunCommands["43"])
                        elseif gun == 0 then
                            sampSendChat(gunCommands["0"])
                        end
                        lastgun = gun
                    end
                end
        ...
        imgui.ToggleButton(u8"Полицейский в менюшке", u8"Снежинка в менюшке", copMenu)
        if copMenu[0] then
...
function sampev.onServerMessage(color, message)
    if message:find("Вы посадили игрока (%w+_%w+) в тюрьму на (.+) минут.") then
        ...
        addLogEntry("Арест", player, nil, nil, duration)
    end
...
imgui.PageButton = function(bool, icon, name, but_wide, but_high)
    ...
    if bool then
        if pool.clock and (os.clock() - pool.clock) < duration then
            ...
            DL:AddRectFilled(imgui.ImVec2(p1.x, p1.y + mainIni.menuSettings.vtpos), imgui.ImVec2((p1.x + but_wide) - wide, p1.y + mainIni.menuSettings.vtpos + but_high),
                0x10FFFFFF, 15, 10)
            DL:AddRectFilled(imgui.ImVec2(p1.x, p1.y + mainIni.menuSettings.vtpos), imgui.ImVec2(p1.x + 5, p1.y + mainIni.menuSettings.vtpos + but_high), ToU32(col))
            DL:AddRectFilled(imgui.ImVec2(p1.x, p1.y + mainIni.menuSettings.vtpos), imgui.ImVec2(p1.x + wide, p1.y + mainIni.menuSettings.vtpos + but_high),
                ToU32(imgui.ImVec4(col.x, col.y, col.z, 0.6)), 15, 10)
        else
            DL:AddRectFilled(imgui.ImVec2(p1.x, (pressed and p1.y + mainIni.menuSettings.vtpos or p1.y + mainIni.menuSettings.vtpos)),
                imgui.ImVec2(p1.x + 10, (pressed and p1.y + but_high - 23 + mainIni.menuSettings.vtpos or p1.y + but_high + mainIni.menuSettings.vtpos)), ToU32(col))
            DL:AddRectFilled(imgui.ImVec2(p1.x, p1.y+mainIni.menuSettings.vtpos), imgui.ImVec2(p1.x + but_wide+2, p1.y + but_high+mainIni.menuSettings.vtpos),--Вот эти десятки
                ToU32(imgui.ImVec4(col.x, col.y, col.z, 0.6)), 15, 10)
        end
    else
        if imgui.IsItemHovered() then
            DL:AddRectFilled(imgui.ImVec2(p1.x+2, p1.y + mainIni.menuSettings.vtpos), imgui.ImVec2(p1.x + but_wide+2, p1.y + but_high + mainIni.menuSettings.vtpos), 0x10FFFFFF, 15, 10)
        end
    end
...
function imgui.Hint(str_id, hint_text, no_trinagle, show_always, y_offset)
    ...
    local rend_window = function(alpha)
        ...
        imgui.Begin('bvsdfs##' .. str_id, _, imgui.WindowFlags.Tooltip + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
...
imgui.OnFrame(
    ...
    function(player)
        ...
        imgui.Begin('Jone', joneV, imgui.WindowFlags.NoDecoration + imgui.WindowFlags.AlwaysAutoResize)
         imgui.ImageURL("https://sun9-73.userapi.com/impf/c622023/v622023770/33fd/T9qIlEYed6o.jpg?size=320x427&quality=96&sign=bfc6230a550a94c075a5b0747a7c6bca&c_uniq_tag=Kl4qcaTNH2y8ypcpjcIMF7CDzDRlSY1rwm8e1dQD504&type=album",
...
imgui.OnFrame(
    ...
    function(player)
        ...
        imgui.Begin(u8" ##patrol_menu", patroolhelpmenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize)
