...
function load_settings()
    ...
    --Smart UK
    local file = io.open(smartUkPath, "r") -- Îòêðûâàåì ôàéë â ðåæèìå ÷òåíèÿ
    if not file then
    ...
    function(player)
        ...
        if imgui.BeginChild('Name', imgui.ImVec2(0, imgui.GetWindowSize().y - 36 - imgui.GetCursorPosY() - imgui.GetStyle().FramePadding.y * 2), true) then
            for i = 1, #tableUk["Text"] do
                imgui.Text(u8(tableUk["Text"][i] .. ' Óðîâåíü ðîçûñêà: ' .. tostring(tableUk["Ur"][i])))
                Uk = #tableUk["Text"]
            end
            imgui.EndChild()
        end
        ...
        if isPatrolActive then
            ...
            if imgui.Button(u8('Çàâåðøèòü'), imgui.ImVec2(100 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                lua_thread.create(function()
                    ...
                    wait(1200)
                    sampSendChat('/r Ïàòðóëèðîâàë ' .. formatPatrolDuration(os.time() - startTime))
                    patrolDuration = 0
...
function sampev.onServerMessage(color, message)
    if message:find("Âû ïîñàäèëè èãðîêà (%w+_%w+) â òþðüìó íà (.+) ìèíóò.") then
        ...
        addLogEntry("Àðåñò", player, nil, duration)
    end
...
function DownloadUk()
    ...
    if url then
        ...
        msg("{FFFFFF} Ïðîèçîøëà îøèáêà. Ïîïðîáóéòå ñêà÷àòü ÓÊ âðó÷íóþ(äëÿ ëþáîãî ñåðâåðà è âûáåðèòå ñâîé).", 0x8B00FF)
    end
end
--Download files END
...
imgui.OnFrame(
    ...
    function(player)
        ...
        imgui.SameLine()
        if imgui.Button(u8'Çàãðóçèòü îáíîâëåíèå', imgui.ImVec2(250 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
...
local mainMenuFrame = imgui.OnFrame(function() return window[0] end, 
    ...
    if page == 1 then
        ...
        imgui.Separator()
        imgui.ToggleButton(u8(mainIni.settings.ObuchalName) .. u8 ' ðàáîòàåò', u8(mainIni.settings.ObuchalName) .. u8 ' îòäûõàåò', joneV)
        if joneV[0] then
        ...
        if imgui.ColoredButton(u8"Óäàëèòü ñêðèïò", 'F94242', 90) then
            imgui.OpenPopup(u8'Âû óâåðåíû ÷òî õîòèòå óäàëèòü ñêðèïò è âñå åãî íàñòðîéêè?')
        end
        if imgui.ColoredButton(u8"Ñáðîñèòü íàñòðîéêè ñêðèïòà", 'F94242', 90) then
            imgui.OpenPopup(u8'Âû óâåðåíû ÷òî õîòèòå óäàëèòü âñå íàñòðîéêè ñêðèïòà?')
        end
        if imgui.ColoredButton(u8"Ïåðåçàãðóçèòü ñêðèïò", 'F94242', 90) then
            thisScript():reload()
        end
        if imgui.BeginPopupModal(u8'Âû óâåðåíû ÷òî õîòèòå óäàëèòü ñêðèïò è âñå åãî íàñòðîéêè?', _) then
            imgui.SetWindowSizeVec2(imgui.ImVec2(750, 300))
            if imgui.Button(u8'Äà', imgui.ImVec2(280, 80)) then
                ...
                msg("Ñêðèïò óäàëåí. Åñëè âû åãî óäàëèëè èç-çà áàãîâ èëè äðóãèõ íåäî÷åòîâ ïîæàëóéñòà ñîîáùèòå îá ýòîì â ëñ @daniel2903_pon")
                thisScript():reload()
                imgui.CloseCurrentPopup()
            end
            imgui.SameLine()
            if imgui.Button(u8'Íåò', imgui.ImVec2(280, 80)) then
                imgui.CloseCurrentPopup()
            end
            imgui.End()
        end
        if imgui.BeginPopupModal(u8'Âû óâåðåíû ÷òî õîòèòå óäàëèòü âñå íàñòðîéêè ñêðèïòà?', _) then
            imgui.SetWindowSizeVec2(imgui.ImVec2(750, 300))
            if imgui.Button(u8'Äà', imgui.ImVec2(280, 80)) then
                deliting_script = true
                os.remove(getWorkingDirectory():gsub('\\','/').."/MVDHelper/buttons.json")
                ...
                os.remove(getWorkingDirectory():gsub('\\','/').."/MVDHelper")
                os.remove(getWorkingDirectory():gsub('\\','/').."/config/MVDHelper.ini")
                msg("Íàñòðîéêè ñêðèïòà ñáðîøåíû.")
                thisScript():reload()
                imgui.CloseCurrentPopup()
            end
            imgui.SameLine()
    ...
    elseif page == 6 then -- Èíôîðìàöèÿ
        ...
        imgui.Text(u8 'Ðàçðàáîò÷èêè: @Sashe4ka_ReZoN, @daniel2903_pon')
        imgui.Text(u8 'ÒÃ êàíàë: t.me/lua_arz')
        if imgui.IsItemClicked() then
            openLink("https://t.me/lua_arz")
        end
        imgui.Text(u8 'Ïîääåðæàòü: Âðåìåííî íå äîñòóïíî')
        imgui.Text(u8 'Ñïîíñîðû: arzfun.com, @Negt, @King_Rostislavia, @sidrusha, @Timur77998, @osp_x, @Theopka')
    end
    imgui.EndChild()
    imgui.End()
end)
...
imgui.OnFrame(
    ...
    function(player)
        ...
        imgui.Begin(u8 "Íà÷àëî", startWindow)
        if start.step == 1 then
            if start.custom == nil then
                imgui.Text(u8"Ïðèâåò! Êàæåòñÿ òû âïåðâûå çàïóñêàåøü ñêðèïò. Äàâàé âìåñòå íàñòðîåì åãî.")
                imgui.Text(u8"Âûáåðè ðåæèì íàñòðîéêè: ")
                if imgui.Button(u8"Àâòîìàòè÷åñêàÿ(ïî òâîåé ñòàòèñòèêå) "..fa["WAND_MAGIC"]) then
                    start.custom = "auto"
                end
                if imgui.Button(u8"Ðó÷íàÿ "..fa["PEN"]) then
                    start.custom = "ruch"
                end

            elseif start.custom == "auto" then
                while not nickname do
                    imgui.TextDisabled(u8"Ïîëó÷àåì èíôîðìàöèþ èç ñòàòèñòèêè...")
                end
                imgui.Text(u8"Èíôîðìàöèÿ èç ñòàòèñòèêè ïîëó÷åíà. Ïðîâåðüòå ïðàâèëüíîñòü")
                imgui.Separator()
                imgui.Text(u8"Âàø Nick_Name:" .. nickname)
                if u8:decode(org) == 'Âû íå ñîñòîèòå â ÏÄ' then
                    imgui.Text(u8"Êàæåòñÿ, âû íå ñîñòîèòå â ÌÞ. Íî âû âñå ðàâíî ìîæåòå èñïîëüçîâàòü õåëïåð.")
                else
                    imgui.Text(u8"Âàøà îðãàíèçàöèÿ: " .. org .. u8"(ñîêðàùåííî - " .. org_g .. ")")
                    imgui.Text(u8"Âàøà äîëæíîñòü: " .. dl .. "[".. rang_n .."]")
                end
                if imgui.Button(u8"Ïðîäîëæèòü "..fa["RIGHT_LONG"]) then
        ...
        elseif start.step == 2 then
            imgui.Text(u8"Îòëè÷íî! Òåïåðü äàâàé íàñòðîèì ãëàâíîå îêíî ñêðèïòà!")
            imgui.Text(u8"Òû ñìîæåøü âûáðàòü ðàçìåðû îêíà, öâåò, ðàçìåðû íàâèãàöèîííîãî ìåíþ è äð.")
            if imgui.Button(u8"Îòêðûòü íàñòðîéêè è îêíî") then
                menuSizes[0] = true
                window[0] = true
            end
            if imgui.Button(u8"Çàâåðøèòü è ïðîäîëæèòü "..fa["RIGHT_LONG"]) then
                window[0] = false
                menuSizes[0] = false
                start.step = 3
            end
        ...
        elseif start.step == 3 then
            imgui.Text(u8"Îòëè÷íî! Òåïåðü äàâàé íàñòðîèì ñàìîå ãëàâíîå äëÿ êîïà - Óãîëîâíûé êîäåêñ.")
            if imgui.Button(u8"Íàñòðîèòü") then
                setUkWindow[0] = not setUkWindow[0]
            end
            if imgui.Button(u8"Ãîòîâî, ïðîäîëæèòü "..fa["RIGHT_LONG"]) then
                setUkWindow[0] = false
                start.step = 4
            end

        elseif start.step == 4 then
            imgui.Text(u8"Óæå ïî÷òè çàêîí÷èëè! Îñòàëîñü ÷óòü-÷óòü")
            imgui.Text(u8"Òåïåðü äàâàé íàñòðîèì àâòîìàòè÷åñêóþ ðï îòûãðîâêó îðóæèÿ.")
            imgui.ToggleButton(u8 'Àâòî îòûãðîâêà îðóæèÿ', u8 'Àâòî îòûãðîâêà îðóæèÿ', autogun)
            if autogun[0] then
                mainIni.settings.autoRpGun = true
                if not deliting_script then saveIni() end
                if imgui.Button(u8 "Îòêðûòü íàñòðîéêè îòûãðîâîê îðóæèé") then
                    gunsWindow[0] = not gunsWindow[0]
                end
            end
            if imgui.Button(u8"Ïðîäîëæèòü "..fa["RIGHT_LONG"]) then
        ...
        elseif start.step == 5 then
            imgui.Text(u8"Ïîçäðîâëÿþ, íàñòðîéêà õåëïåðà çàâåðøåíà. Âñå ïðî÷èå ìåëêèå íàñòðîéêè òû ìîæåøü èçìåíèòü âî âêëàäêå íàñòðîêè â ãëàâíîì ìåíþ.")
            imgui.Text(u8"Åñëè õî÷åøü ïîñìîòðåòü îáó÷åíèå ïî ãëàâíîìó ìåíþ, òî ïîïðîñè Ìàñòóðáåêà!")
            imgui.ToggleButton(u8(mainIni.settings.ObuchalName) .. u8 ' ðàáîòàåò', u8(mainIni.settings.ObuchalName) .. u8 ' îòäûõàåò', joneV)
            if imgui.Button(fa["XMARK"].. u8" Çàêðûòü ýòî îêíî " .. fa["XMARK"]) then
                startWindow[0] = false
            end
        end
        imgui.End()
    ...
    while true do
        ...
        if search then
            ...
            if res then
                ...
                msg("Èãðîê äîëæåí áûòü â çîíå ñòðèìà!")
                search = false
            end
        end
    end
    ...
    if id ~= nil then
        ...
        if search then
            ...
            if playerId ~= nil and sampIsPlayerConnected(playerId) then
                ...
                search = false
                msg('Ëèáî âû ââåëè íåïðàâèëüíûé àðãóìåíò, ëèáî èãðîê íå îíëàéí.') 
            end
        else
            msg('Îñòàíîâèë ïîèñê.') 
        end
